...
class OptimizedLSTMTrainer:
    ...
    def optimized_lstm_loss(self, q_values, actions, targets, weights=None, mask=None, temporal_weights=None):
        ...
        q_taken = torch.gather(q_values, 2, actions.unsqueeze(-1)).squeeze(-1)
        ...
        if getattr(lstm_config_copy, 'use_huber_loss', True):
            ...
            loss_per_timestep = F.smooth_l1_loss(q_taken_f32, targets_f32, reduction='none', beta=huber_delta)
        else:
    ...
    def train_step(self, batch_data, step):
        ...
        with torch.amp.autocast(device_type="cuda", dtype=torch.float16, enabled=self.use_mixed_precision):
            ...
            q_values, _ = self.online_network(img_seq, float_seq)
            ...
            with torch.no_grad():
                ...
                q_next, _ = self.target_network(next_img_seq, next_float_seq)
        ...
        if (step + 1) % self.use_gradient_accumulation == 0:
            ...
            # For gradient accumulation steps, compute norm without unscaling
            # to avoid interfering with the scaler
            total_norm = 0.0
    ...
    def advanced_gradient_clipping(self):
        ...
        # Debug: Check if gradients exist and their magnitudes
        grad_count = 0
        total_grad_magnitude = 0.0
        for name, param in self.online_network.named_parameters():
            if param.grad is not None:
                grad_count += 1
                grad_magnitude = param.grad.data.norm().item()
                total_grad_magnitude += grad_magnitude
                # if grad_count <= 3:  # Print first few gradients for debugging
                #     print(f"DEBUG: {name} grad norm: {grad_magnitude:.6f}")

        if grad_count == 0:
            print("DEBUG: No gradients found!")
            return 0.0

        # Compute gradient norm
        total_norm = torch.nn.utils.clip_grad_norm_(
            self.online_network.parameters(),
            max_norm=getattr(config_copy, 'clip_grad_norm', 30.0)
        )
        
        # Additional gradient value clipping
        torch.nn.utils.clip_grad_value_(
            self.online_network.parameters(),
            clip_value=getattr(config_copy, 'clip_grad_value', 1000.0)
        )
